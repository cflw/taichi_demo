...
c圆形层数 = 4
c圆形点数 = c圆形边数 * c圆形层数
c圆形线条数 = 3 * c圆形点数 - 2 * c圆形边数	#总线条数,每一层线条数等于点数,每两层之间线条数为两层点数之和
...
@ti.kernel
def init():	#生成圆环
	v初始速度 = t向量2(ti.random(), ti.random())
	#点
	for i, j in ti.ndrange(c圆形层数, c圆形边数):
		v点开始 = i * c圆形边数
		v半径 = c圆形半径 / c圆形层数 * (i + 1)
		k = v点开始 + j
		v方向 = 二PI / c圆形边数 * j
		# v方向 += PI / c圆形边数 * i	#每层旋转一点角度
		v点[k].m位置 = ti.Vector([ti.sin(v方向) * v半径 + 0.5, ti.cos(v方向) * v半径 + 0.5])
		v点[k].m速度 = v初始速度
		v点[k].m新速度 = t向量2(0, 0)
	#线(弹簧)
	for i, j in ti.ndrange(c圆形层数, c圆形边数):	#正多边形
		v点开始 = i * c圆形边数
		k = v点开始 + j
		if j != c圆形边数 - 1:
			v线[k] = new_line(k, k+1)
	...
	for i, j in ti.ndrange(c圆形层数-1, c圆形边数):	#两层之间的线条
		v线开始 = c圆形点数 + i * c圆形边数 * 2
		v内点开始 = i * c圆形边数
		v外点开始 = v内点开始 + c圆形边数
		k = v线开始 + j * 2
...
def compute(dt: float):
	for i in v线:	#弹簧
		...
		v点[v线0.a].m新速度 += v速度变化1
		v点[v线0.b].m新速度 -= v速度变化1
	for i in v点:	#运动,边框反弹
		v点[i].m新速度.y -= g * dt
		v点[i].m速度 += v点[i].m新速度
		v点[i].m新速度 = t向量2(0, 0)
